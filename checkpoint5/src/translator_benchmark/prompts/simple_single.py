...
#аналогично батчу профайлер под запрос перевода одной строки
class SimpleSinglePromptProfile:

    def __init__(self, config: PromptConfig):
        self.config = config

    def build_batch_messages(self, texts: List[str], src_lang: str, dst_lang: str) -> List[Dict[str, str]]:
        raise NotImplementedError("Simple single prompt does not support batch mode")
        ...
        if system_prompt:
            try:
                system_content = system_prompt.format(
                    text=text, src_lang=src_lang, dst_lang=dst_lang
                )
            except KeyError:
...
def build_simple_single_profile(config: PromptConfig) -> SimpleSinglePromptProfile:
    """
    Args:
        config: Prompt configuration.

    Returns:
        SimpleSinglePromptProfile instance.
    """
    return SimpleSinglePromptProfile(config)