...
Всего на странице мастерской найдено **34 116** модификаций, из которых реально скачано **1 599** (≈4.7 %).
Из них обнаружено и извлечено **19 653** файлов локализаций.
...
**Тип данных:** пары ключ–строка перевода.  
Помимо корректных записей встречаются и некорректные элементы, не прошедшие фильтрацию, включая:
- битые строки (ошибки парсинга, обрывы YAML);
- служебные символы, добавленные Paradox и относящиеся к внутренней разметке, а не к переводу;
- комментарии переводчиков;
- текстовые инструкции и руководства по замене содержимого файлов, не оформленные как комментарии и не относящиеся к переводу.
...
- Без реальной детекции по содержимому невозможно надёжно определить язык файла —  **особенно**, если мододел по каким-то причинам **нигде** не указал, что язык у него тайский.  (А на прошлых этапах файлов у которых язык не свелся к базовому словарю языков было 3 и они были на англе)
  ...
  После всех языковых преобразований получаем набор файлов с корректно расставленными языками, дальнейший анализ будет проивзодиться на них.
...
- Вычисляется косинусное сходство эмбеддингов (semantic_parallel_score, рассчитанный на основе модели `paraphrase-multilingual-MiniLM-L12-v2`, показывает насколько переводы совпадают по смыслу).
...
Датасет (в фильтрованной версии, где удалены непереведённые и дублирующиеся пары)
хорошо подходит для проверки качества перевода и оценки моделей. Сохранность плейсхолдеров большая, а если от перевода семантическое сходство вырастет то будет еще лучше. По сути мы можем получить базовый метод оценки правильности сделанного перевода, для обучающего датасета.
...
На графике ниже каждая точка представляет отдельную строку локализации, а цвета соответствуют различным кластерам по типу содержимого (см. далее раздел **«Интерпретация кластеров»**).
